      ...
      <div className="h-auto flex gap-4 overflow-x-scroll pb-4">
        {meals && meals.length > 0 ? (
          ...
          {meals.map((meal: any, index: any) => (
            <MealCard active={false} title={meal.name} image={"https://images.unsplash.com/photo-1482049016688-2d3e1b311543?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=710&q=80"} seasons={meal.season} ingredients={meal?.ingredients?.length} onClick={onMealClicked} key={index} />
          ))}
          </>
