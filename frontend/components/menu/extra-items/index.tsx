...
const ItemWrapper = styled.li`
  ...
  background-color: ${props => (props.checked ? colors.secondary : colors.secondary_light)};
`;
...
const Item = ({ item }) => {
  ...
  return (
    <ItemWrapper checked={checked}>
      {checked ? (
        <IoCheckbox color={colors.primary} onClick={() => handleCheck(item)} size={22} />
      ) : (
        <IoCheckboxOutline color={colors.grey_light} onClick={() => handleCheck(item)} size={22} />
      )}
      <ItemName
    ...
    <Wrapper>
      {!add ? (
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          <Input height="2rem" width="" placeholder="Item Name" {...register('name', {})} />
          <UtilityWrapper>
